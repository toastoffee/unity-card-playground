...
public static class CurveUtils {
  ...
  //public static Quaternion RotateFromArmToward(Vector3 armLocalPosX0Z, Vector3 targetLocalPosX0Z) {
  //  var armLocalPos = armLocalPosX0Z.XZ();
  //  var targetLocalPos = targetLocalPosX0Z.XZ();
  //  return Quaternion.FromToRotation(armLocalPos, targetLocalPos);
  //}

  public static Quaternion RotateFromArmToward(Vector3 toWardAxis, Vector3 armLocalPos, Vector3 targetLocalPos) {
    var armProjOnBase = Vector3.ProjectOnPlane(armLocalPos, toWardAxis);
    var armProjOnAxis = Vector3.Project(armLocalPos, toWardAxis);
    var toTargetDist = targetLocalPos.magnitude;
    var toTargetAxisLength = Mathf.Sqrt(toTargetDist * toTargetDist - armProjOnBase.magnitude * armProjOnBase.magnitude);
    var toTargetAxis = toTargetAxisLength * armProjOnAxis.normalized;
    var toTarget = armProjOnBase + toTargetAxis;
    return Quaternion.FromToRotation(toTarget, targetLocalPos);
  }
  public struct ADSR {
    public float a;
    public float d;
