...
public class AutoLoader : ScriptableObject {
	...
	public T Load<T>(string id) where T : UObject {
		return FindObjectByCondition<T>(x => x.name.ToLower() == id.ToLower());
	}
	...
	public void EditorOnlyReload() {
		...
		TwFileUtil.TryParsePath(selfPath, out var diretory, out _);
		var newName = "#" + diretory.Replace('/', '_');
		AssetDatabase.RenameAsset(selfPath, newName);
		name = newName;

		TwFileUtil.TryParsePath(selfPath, out var direcotry, out _);
		var allUObjectPath = TwFileUtil.FindAssetPathsOfType<UObject>(direcotry);
...
class AutoObjectCachePostprocessor : AssetPostprocessor {
	...
	private static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths, bool didDomainReload) {
		...
		foreach (var path in addedPath) {
			if (!TwFileUtil.TryParsePath(path, out var directory, out var fileName)) {
				return;
			}
	...
	private static bool TryFindCache(string directory, out AutoLoader cache) {
		cache = null;
		var foundPaths = TwFileUtil.FindAssetPathsOfType<AutoLoader>(directory);
